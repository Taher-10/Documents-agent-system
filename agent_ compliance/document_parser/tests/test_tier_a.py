...
def _make_page_map(path: Path, n_pages: int) -> PageMap:
    return PageMap(
        total_pages=n_pages,
        file_format="pdf",
        quality_tier="A",
...
def test_returns_one_result_per_page(pdf_path):
    results = extract_tier_a(pdf_path, _make_page_map(pdf_path, 2))
...
def test_page2_falls_back_to_fitz(pdf_path):
    """Page 2 has only 2 chars of text → fitz fallback fires."""
    results = extract_tier_a(pdf_path, _make_page_map(pdf_path, 2))
    assert results[1].extraction_method == "fitz"


def test_tables_field_is_list(pdf_path):
    results = extract_tier_a(pdf_path, _make_page_map(pdf_path, 1))
